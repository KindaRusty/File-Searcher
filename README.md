...
>
> _Một ứng dụng desktop song ngữ mạnh mẽ để tìm kiếm văn bản bên trong tài liệu và hình ảnh._
...
1. **Python 3.6+**
2. **Tesseract OCR Engine:** This is _crucial_ for searching inside images. The script **cannot** install this for you.
   - **Windows:**
     - Download the installer from [UB-Mannheim Tesseract GitHub](https://github.com/UB-Mannheim/tesseract/wiki).
...
1. **Download the code:** Clone this repository or download the `.py` file to your computer.
2. **Open a terminal:** Open `Command Prompt` (on Windows) or `Terminal` (on macOS/Linux).
3. **Navigate to the folder:** Use the `cd` command to go to the directory where you saved the file.
   ```
...
4. **Run the script:**
   ...
   ```
   _*(Replace `your_script_name.py` with the actual name of the file.)*_
5. **Automatic Installation:** The first time you run the script, it will check for required Python libraries (`python-docx`, `PyMuPDF`, `pytesseract`, `Pillow`) and install them if they are missing. Please ensure you have an active internet connection.

### **How to Use the Application**

1. _Select a folder to search in:_ Click the **"Browse..."** button to choose the root folder you want to search.
2. _Enter the phrase to find:_ Type the exact word or phrase you are looking for in this text box.
3. _Select file types to search:_ Check the boxes for the file extensions you want to include in the search.
4. _Start Search:_ Click this button to begin the search process. The UI will remain responsive.
5. _Reset:_ Clears all inputs and results to start a new search.
6. _Search Results:_ This area will display a real-time log of the files being scanned and any matches found.
7. _Language Toggle:_ The button in the top-right corner allows you to switch the UI language.
...
1. **Python 3.6+**
2. **Tesseract OCR Engine:** Đây là thành phần **cốt lõi** để tìm kiếm trong ảnh. Script _không thể_ tự cài đặt Tesseract cho bạn.
   - **Windows:**
     - Tải bộ cài đặt từ [GitHub của Tesseract tại UB-Mannheim](https://github.com/UB-Mannheim/tesseract/wiki).
   ...
   - **Linux (Debian/Ubuntu):**
     - Sử dụng apt-get:
       ...
       sudo apt-get install tesseract-ocr
       ```
     - Để nhận dạng tiếng Việt, cài thêm gói ngôn ngữ:
       ```
...
1. **Tải mã nguồn:** Clone repository này hoặc tải file `.py` về máy tính của bạn.
2. **Mở terminal:** Mở `Command Prompt` (trên Windows) hoặc `Terminal` (trên macOS/Linux).
3. **Đi đến thư mục:** Sử dụng lệnh `cd` để di chuyển đến thư mục bạn đã lưu file.
   ```
...
4. **Chạy script:**
   ...
   ```
   _*(Thay thế `ten_file_cua_ban.py` bằng tên thật của file.)*_
5. **Cài đặt tự động:** Lần đầu tiên chạy, script sẽ kiểm tra các thư viện Python cần thiết (`python-docx`, `PyMuPDF`, `pytesseract`, `Pillow`) và tự động cài đặt nếu thiếu. Hãy đảm bảo bạn có kết nối Internet.

### **Cách sử dụng Ứng dụng**

1. _Chọn thư mục để tìm kiếm:_ Nhấn nút **"Chọn Thư Mục..."** để mở cửa sổ và chọn thư mục gốc bạn muốn tìm kiếm.
2. _Nhập cụm từ cần tìm:_ Gõ từ hoặc cụm từ chính xác bạn muốn tìm vào ô văn bản này.
3. _Chọn loại file để tìm kiếm:_ Tích vào các ô tương ứng với các loại file bạn muốn tìm kiếm.
4. _Bắt đầu Tìm kiếm:_ Nhấn nút này để bắt đầu quá trình tìm kiếm. Giao diện sẽ luôn phản hồi.
5. _Làm lại:_ Xóa toàn bộ các lựa chọn và kết quả để bắt đầu một lượt tìm kiếm mới.
6. _Kết quả tìm kiếm:_ Khu vực này sẽ hiển thị nhật ký thời gian thực về các file đang được quét và các kết quả tìm thấy.
7. _Chuyển đổi Ngôn ngữ:_ Nút ở góc trên bên phải cho phép bạn chuyển đổi ngôn ngữ của toàn bộ giao diện.
